...
const unpackZip = async (filePath) => {
    ...
    return new Promise((resolve, reject)=>{
        SHELL.exec(`unzip -o -d ${tempDir} ${filePath}`, {silent: false}, (code, stdout, stderr)=>{
            if (code !== 0) {
                reject(stderr);
            } else {
                resolve(tempDir);
...
const uploadDeploymentPackageToS3 = async (sourceDir, bucket, keyPrefix) => {
...
const loadTemplateFromS3 = async (bucket, keyPrefix, templateKey) => {
    return new Promise((resolve, reject) => {
        const filePath = `/tmp/${genString(32)}`;
        const command = `aws s3api get-object --bucket ${bucket} --key "${PATH.join(keyPrefix, templateKey)}" ${filePath}`;
...
const genString = (length, specialChars = []) => {
    ...
    let charCodes = [], size = ranges.length + (specialChars.length > 0 && 1 || 0);
    for (let i = 0; i < length; i ++) {
        let s = Math.floor(Math.random() * size);
        let range = ranges[s];
        if (!range) {
            charCodes.push(specialChars[Math.floor(Math.random() * specialChars.length)].charCodeAt(0));
        } else {
...
const genPassword = (length, containSymbol = false) => {
    const specialChars = containSymbol && ['!', '@', '#', '$', '%', '^', '&', '*', '+', '-'] || [];
    return genString(length, specialChars);
}

const fetchAWSAvailabilityZones = async () => {
    return new Promise((resolve, reject) => {
        ...
        SHELL.exec(command, { silent: false }, (code, stdout, stderr) => {
            if (code !== 0) {
                ...
                try {
                    const zoneName = JSON.parse(stdout).AvailabilityZones
                        .filter(z => z.State === 'available').map(zone => zone.ZoneName);
                    resolve(zoneName);
                } catch (error) {
                    reject(error);
                }
...
const genAvailabilityZones = async (num) => {
    ...
    if (azones.length < num) {
        throw new Error('parameter value generation error:' +
...
const createCloudFormationStack = async (stackName, templateUrl, paramFilePath, disableRollback = true) => {
    return new Promise((resolve, reject)=>{
        const command = `aws cloudformation create-stack --stack-name ${stackName}`+
...
const parseParams = async (paramJSON, cfnTemplateParams, paramVariableMap, awsVars) => {
    return Promise.all(paramJSON.map(async (param) => {
        if (paramUseTemplateDefault(param)) {
            ...
            console.info(`Overridden value: [${param.ParameterValue}], for parameter: ${param.ParameterKey}`);
        }
        else {
            ...
            if (v.startsWith('$[cd_randchar')) {
                ...
                console.info(`Generated password: [${param.ParameterValue}], for parameter: ${param.ParameterKey}`);
            }
        }
        return param;
    }));
...
const genOverriddenParamMap = (inputString) => {
    const a = inputString.split(' ').map(nvp => {
        const [, name, value] = /Name=(\S+),Value=(\S+)/ig.exec(nvp) || [null, null, null];
        return [name, value];
...
const create = async (bucket, keyPrefix, templatePath, command) => {
    ...
    const paramVariableMap = genOverriddenParamMap(command.override);
    ...
    try {
        ...
        paramJSON = await parseParams(paramJSON, templateJSON.Parameters, paramVariableMap, { region: command.awsRegion });
...
program
    ...
    .command('deploy <package-file-path>')
        .description('Deploy a Autoscale deployment package. run deploy -h or --help for command specific help.')
        .requiredOption('-b, --bucket <value>', 'the S3 bucket to upload the deployment package.')
        .requiredOption('-k, --access-key-id <value>', 'provided access key id for a programatic IAM user access to AWS.')
        .requiredOption('-s, --secret-access-key <value>', 'provided secret access key for a programatic IAM user access to AWS.')
        .requiredOption('-g, --aws-region <region-code>', 'the region to create stacks.')
        .option('-p, --key-prefix <value>', 'the sub directory in the specifed S3 bucket to upload the deployment package.')
    .action(deploy);

program
    .command('create <bucket> <key-prefix> <path-to-template>')
    .description('Create a stack for Autoscale using a deployed deployment package. run create -h or --help for command specific help.')
    .requiredOption('-k, --access-key-id <value>', 'provided access key id for a programatic IAM user access to AWS.')
    .requiredOption('-s, --secret-access-key <value>', 'provided secret access key for a programatic IAM user access to AWS.')
    .requiredOption('-g, --aws-region <region-code>', 'the region to create stacks.')
    .requiredOption('-K, --ssh-key <value>', 'the ssh key pairs to login into an ec2 device. must specify one that exists in the specified region.')
    .option('-a, --param-file <file-path>', 'a json file for all input parameters to deploy the stack. These parameters override all default ones in the specified template.')
    .option('-o, --override <list>', 'a list of name-value pair to override the template parameter. format: "Name=param-name,Value=new-value[ Name=param-name,Value=new-value[ ...]]"')
    .action(create)
